...
## Requirements
- Read the Harbor trial identifier from `/logs/artifacts/trial_id` and use it to derive a unique bucket name `harbor-ttl-${trial_id}`.
...
3. Derive the bucket name as `harbor-ttl-${trial_id}`.
...
- Bucket name MUST be exactly `harbor-ttl-${trial_id}` where `${trial_id}` is the contents of `/logs/artifacts/trial_id` (trimmed of trailing whitespace). Do NOT hardcode any other suffix.
