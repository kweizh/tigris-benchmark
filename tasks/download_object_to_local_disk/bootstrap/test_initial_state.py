...
EXPECTED_OBJECT_KEY = "assets/welcome.md"
EXPECTED_OBJECT_BYTES = b"# Welcome to Tigris\n"
...
def _wait_for_seed_sentinel(timeout_sec=180):
    """Wait for the entrypoint seed script to finish creating the bucket and
    uploading the welcome.md object."""
...
def test_local_welcome_md_does_not_yet_exist():
    target = os.path.join(PROJECT_DIR, "welcome.md")
    assert not os.path.exists(target), (
        f"{target} must NOT exist at the start of the task; it is produced by "
        "the agent by downloading the object out of Tigris."
    )


def test_entrypoint_seed_completed():
    """The container entrypoint must have created the bucket and uploaded the
    `assets/welcome.md` object before the agent runs."""
...
def test_seeded_bucket_exists_in_tigris():
    """The pre-seeded bucket must be visible to the Tigris CLI."""
    _wait_for_seed_sentinel()
    ...
    assert bucket_name in names, (
        f"Expected pre-seeded bucket {bucket_name!r} in Tigris, but it was not "
        f"in the bucket list. Got: {names}"
    )


def test_seeded_object_exists_in_bucket():
    """The pre-seeded `assets/welcome.md` object must already be in the bucket."""
    _wait_for_seed_sentinel()
    bucket_name = _expected_bucket_name()
    result = subprocess.run(
        ["tigris", "ls", f"t3://{bucket_name}/assets/"],
        capture_output=True,
        text=True,
        env=_tigris_env(),
        cwd=PROJECT_DIR,
    ...
    assert result.returncode == 0, (
        f"'tigris ls t3://{bucket_name}/assets/' failed: returncode="
        f"{result.returncode} stderr={result.stderr!r} stdout={result.stdout!r}"
    )
    assert "welcome.md" in result.stdout, (
        f"Expected pre-seeded object 'assets/welcome.md' to be visible under "
        f"t3://{bucket_name}/assets/, but it was not in the listing. "
        f"Got stdout:\n{result.stdout}"
    )
