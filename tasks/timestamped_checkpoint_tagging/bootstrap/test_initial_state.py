...
def test_build_id_file_exists_with_v42():
    assert os.path.isfile(BUILD_ID_FILE), (
        f"build_id.txt not found at {BUILD_ID_FILE}; the pre-task build id file is missing."
    )
    with open(BUILD_ID_FILE) as f:
        content = f.read().strip()
    assert content == "v42", (
        f"Expected {BUILD_ID_FILE} to contain 'v42', got: {content!r}"
    )
