...
3. Derive the bucket name as `harbor-cleanup-${trial_id}`.
...
- Bucket name MUST be exactly `harbor-cleanup-${trial_id}` where `${trial_id}` is the contents of `/logs/artifacts/trial_id` (trimmed of trailing whitespace). Do NOT hardcode any other suffix.
