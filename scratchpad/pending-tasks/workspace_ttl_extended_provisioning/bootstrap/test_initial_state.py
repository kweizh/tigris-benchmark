...
def test_workspace_json_not_yet_created():
    out_path = os.path.join(PROJECT_DIR, "workspace.json")
    assert not os.path.exists(out_path), (
        f"workspace.json should not exist before the task runs (found at {out_path})."
    )


def test_index_ts_not_yet_created():
    idx_path = os.path.join(PROJECT_DIR, "index.ts")
    assert not os.path.exists(idx_path), (
        f"index.ts should not exist before the task runs (found at {idx_path})."
    )
